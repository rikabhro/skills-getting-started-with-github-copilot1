    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsSection = details.participants.length > 0
          ...
          : `<div class="participants-section no-participants-section">
                <p>Participants:</p>
  ...
  activitiesList.addEventListener("click", async (event) => {
    ...
    try {
      const response = await fetch(
        `/activities/${encodeURIComponent(activityName)}/participant?email=${encodeURIComponent(participantEmail)}`,
        {
          method: "DELETE",
        }
      );
